...
void PmergeMe::sort(int argc)
{
    ...
    sortContainer(_vec, 0, _vec.size() - 1);
    std::clock_t endVec = clock();

    std::clock_t beginDeq = std::clock();
    sortContainer(_deq, 0, _deq.size() - 1);
...
template <typename Container>
void PmergeMe::sortContainer(Container& container, unsigned int start, unsigned int end)
{
    if (end - start + 1 <= 2)
        insertContainer(container, start, end);
    else if (end - start > 1)
    {
        unsigned int middle = start + ((end - start) / 2);
        sortContainer(container, start, middle);
        sortContainer(container, middle + 1, end);
        mergeContainer(container, start, end);
    }
}

template <typename Container>
void PmergeMe::insertContainer(Container& container, unsigned int start, unsigned int end)
{
...
template <typename Container>
void PmergeMe::mergeContainer(Container& container, unsigned int start, unsigned int end)
{
    Container tmp;
